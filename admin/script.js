...
function setupCustomSelect(wrapperId, inputId, textId) {
    ...
    window.addEventListener('click', () => wrapper.classList.remove('open'));
}

// --- Funções Auxiliares Visuais ---
function mostrarErroSelect(wrapperId) {
    const wrapper = document.getElementById(wrapperId);
    const trigger = wrapper.querySelector('.custom-select-trigger');
...
document.getElementById("product-form").addEventListener("submit", async (e) => {
    ...
    if (!e.target.checkValidity()) {
        ...
        inputs.forEach(input => {
            if (!input.validity.valid && window.mostrarErro) window.mostrarErro(input);
        });
        hasError = true;
    }
...
document.getElementById('edit-form').addEventListener('submit', async (e) => {
    ...
    if (!categoria) {
        mostrarErroSelect("wrapper-edit-categoria");
        hasError = true;
    }
...
function renderizarPedidos(pedidos) {
    ...
    lista.innerHTML = "";
    if(!pedidos || pedidos.length === 0) { lista.innerHTML = "<p style='padding:20px; color:#888;'>Sem vendas.</p>"; return; }
    pedidos.forEach(p => {
        ...
        card.innerHTML = `<div class="order-header"><span class="order-id">#${p.id}</span><select onchange="atualizarStatus(${p.id}, this.value)" class="status-select">${statusOptions}</select></div><div class="client-info"><strong>${p.cliente_nome}</strong><br>${end.rua}<br>R$ ${p.total}</div><div class="order-items">${itensHtml}</div>`;
...
function renderizarProdutos(produtos) {
    ...
    tbody.innerHTML = "";
    if (!produtos || produtos.length === 0) { tbody.innerHTML = `<tr><td colspan="5" style="text-align:center; padding:20px;">Vazio.</td></tr>`; return; }
    produtos.forEach(p => {
        const cores = p.cores ? JSON.parse(p.cores).length : 0;
        const tr = document.createElement("tr");
        tr.innerHTML = `<td><img src="${p.imagem}" class="prod-img-mini"></td><td>${p.nome}</td><td>R$ ${Number(p.preco).toFixed(2)}</td><td style="font-size:0.8rem; color:#aaa">${cores} cores | ${p.categoria}</td><td><button class="btn-action btn-edit" onclick="abrirModalEdicao(${p.id})">Editar</button><button class="btn-action btn-delete" onclick="deletarProduto(${p.id})">Excluir</button></td>`;
        tbody.appendChild(tr);
    });
}

window.deletarProduto = function(id) { idParaDeletar = id; document.getElementById('confirm-modal').classList.remove('hidden'); }
...
function mostrarErroCustomizado(elementName, formId, msg) {
    ...
    if(container.querySelector('.error-msg-chk')) return;
